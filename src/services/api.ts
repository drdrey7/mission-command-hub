...
/* Agents */
export const getAgents = (): Promise<Agent[]> => USE_MOCK ? delay(mockAgents) : http("/agents");

/* Tasks */
export const getTasks = (): Promise<Task[]> => USE_MOCK ? delay(mockTasks) : http("/tasks");
...
export const getMissions = (): Promise<Mission[]> => USE_MOCK ? delay(mockMissions) : http("/missions");
export const createMission = async (input: NewMissionInput): Promise<Mission> => {
  ...
  return http("/missions", { method: "POST", body: JSON.stringify(input) });
};
export const abortMission = (id: string) =>
  USE_MOCK ? delay({ ok: true }) : http(`/missions/${id}/abort`, { method: "POST" });

/* VPS */
export const getVpsNodes = (): Promise<VpsNode[]> => USE_MOCK ? delay(mockNodes) : http("/vps/nodes");
...
export const getAuditTrail = (limit = 50): Promise<ActivityEvent[]> =>
  USE_MOCK ? delay(mockActivity) : http(`/audit?limit=${limit}`);
...
export const getNotifications = (): Promise<Notification[]> =>
  USE_MOCK ? delay(mockNotifications) : http("/notifications");
...
export const sendChat = async (agent: AgentKey, messages: ChatMessage[]) => {
  ...
  return http<{ reply: string }>(`/chat/${agent}`, { method: "POST", body: JSON.stringify({ messages }) });
};
...
export const getFail2banJails = (): Promise<Fail2banJail[]> =>
  USE_MOCK ? delay(mockJails) : http("/fail2ban/jails");

export const getFail2banBanned = (): Promise<BannedIp[]> =>
  USE_MOCK ? delay(mockBanned) : http("/fail2ban/banned");

export const getFail2banStats = (): Promise<Fail2banStats> =>
  USE_MOCK ? delay(mockStats) : http("/fail2ban/stats");
