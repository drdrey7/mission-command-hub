...
export const AgentChat = ({ externalAgent, open: openProp, onOpenChange, embedded = false }: AgentChatProps = {}) => {
  ...
  const content = (
    <>
      ...
      <div
        ...
        style={{ paddingBottom: `calc(env(safe-area-inset-bottom) + ${embedded ? "0px" : `${keyboardInset}px`} + 0.75rem)` }}
      >
        {voiceStatus && (
  ...
  if (embedded) {
    return (
      <div className="flex h-[calc(100dvh-11rem)] min-h-[560px] flex-col overflow-hidden rounded-2xl border border-border/60 bg-background/70 shadow-panel md:h-[calc(100dvh-8rem)]">
        {content}
      </div>
    );
  }
